...
const burgerIcon = () => (
  <Tooltip title="NextBurger - Fues US®">
    <Link href="/">
      <svg width="26" height="26" style={{ marginLeft: '-6px' }} viewBox="0 0 256 320" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default function SideNavigation({isCollapsed, setIsCollapsed, isDark, setIsDark}) {
  ...
  const useStyles = makeStyles((theme) => ({
    ...
    sideNavLink: {
      ...
      paddingRight: '2rem',
      '&:hover': {
        borderLeft: '3px solid rgba(77,79,83,0.5)',
        textDecoration: 'none',
      },
      '&.active': {
        borderLeft: '3px solid rgba(77,79,83,1)',
        textDecoration: 'none',
      }
    },
    ldIcon: {
  ...
  return (
    <Grid item sm={12} md={6} className={classes.sideContainer}>
      <Box m={0} className={classes.navBox}>
        <Container className={classes.sideControl}>
          { burgerIcon() }
          {isCollapsed? (
            <IconButton color="secondary" onClick={() => { setIsCollapsed(!isCollapsed) }} aria-label="collapse" >
              <LastPageSharp className={classes.ldIcon}/>
            </IconButton>
            ):(
              <IconButton color="secondary" onClick={() => { setIsCollapsed(!isCollapsed) }} aria-label="open" >
        ...
        {!isCollapsed ? (
          <Typography variant="overline" component="h1" gutterBottom className={classes.brand}>
            Nextburger
          </Typography>
        ): (
          <Typography variant="overline" component="h1" gutterBottom className={classes.brand}>
        ...
        <Container disableGutters className={classes.sideLinks}>
          <Tooltip title="Home">
            <Link href="/" color="inherit" activeClassName="active" className={classes.sideNavLink}>
              <FingerprintSharp style={{ color:"#4d4f53" }}/>&nbsp;{!isCollapsed?'Home':''}
            </Link>
          </Tooltip>
          <Tooltip title="Example">
            <Link href="/example" color="inherit" activeClassName="active" className={classes.sideNavLink}>
              <FiberSmartRecordSharp style={{ color:"#4d4f53" }}/>&nbsp;{!isCollapsed?'Example page':''}
            </Link>
          </Tooltip>
          <Tooltip title="Exposure">
            ...
            {isDarkTheme?<ExposurePlus1Sharp style={{ color:"#4d4f53" }}/>:<ExposureNeg1Sharp style={{ color:"#4d4f53" }}/>}&nbsp;{!isCollapsed?(isDarkTheme?'Light mode':'Dark mode'):''}
            </Link>
          </Tooltip>
          <Tooltip title="Share">
            <Link href="/about" color="inherit" activeClassName="active" className={classes.sideNavLink}>
              <ShareSharp style={{ color:"#4d4f53" }}/>&nbsp;{!isCollapsed?'Share':''}
            </Link>
          </Tooltip>
        </Container>
      </Box>
